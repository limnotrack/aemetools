...
#' values, negated so that lower values indicate a better fit - the
#' convention expected of `FUN_list` entries by \code{\link{calib_aeme}}
#' and \code{\link{run_and_fit}}, which minimise the returned value.
#'
...
#' observed and modelled values, negated so that lower values indicate a
#' better fit - the convention expected of `FUN_list` entries by
#' \code{\link{calib_aeme}} and \code{\link{run_and_fit}}, which minimise
#' the returned value.
#'
...
kge <- function(df) {
  ...
  fit <- 1 - sqrt((r - 1)^2 + (alpha - 1)^2 + (beta - 1)^2)
  -1 * fit
}
...
#' generally the recommended default over the original KGE.
#'
#' @inheritParams nse
#'
#' @return numeric; `-1 * KGE'`.
...
kge_prime <- function(df) {
  ...
  fit <- 1 - sqrt((r - 1)^2 + (gamma - 1)^2 + (beta - 1)^2)
  -1 * fit
}
...
#' suitable for variables that can be negative.
#'
#' @inheritParams nse
#'
#' @return numeric; `-1 * KGE` calculated on `log1p(obs)`/`log1p(model)`.
#'
#' @examples
#' df <- data.frame(obs = c(1, 2, 3, 4), model = c(1.1, 2.1, 2.9, 4.2))
#' log_kge(df)
...
#' unlike \code{\link{nse}}/\code{\link{kge}} - it is returned as-is, with
#' no negation needed to match `FUN_list`'s minimise-the-returned-value
#' convention.
#'
...
#' dimensionless metric such as \code{\link{nse}} or \code{\link{kge}} when
...
#' worsens, so - unlike \code{\link{nse}}/\code{\link{kge}} - it is
#' returned as-is, with no negation needed to match `FUN_list`'s
#' minimise-the-returned-value convention.
#'
...
#' *either* direction, so - unlike \code{\link{nse}}/\code{\link{kge}} - it
#' is returned as-is, with no negation needed to match `FUN_list`'s
#' minimise-the-returned-value convention. The absolute value is used
