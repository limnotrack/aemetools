...
run_aeme_param <- function(aeme_data, param, model, path, mod_ctrls,
  ...
  if (!model %in% param[["model"]])
    stop("No parameters in 'param' for ", model, ".")

  all_p <- param[param$model == model, ] # Subset parameters to model specific

  # Scale met data ----
  if ("met" %in% all_p[["file"]]) {

    # Read in meteo file ----
    met_idx <- which(param$model == model & param$file == "met")
  ...
  if (model == "dy_cd") {
    cfg_file <- list.files(file.path(lake_dir, model),
                           pattern = "cfg")
