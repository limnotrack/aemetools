...
# tmap settings
tmap_mode("view")
tmap_options(basemaps = "Esri.WorldImagery", check.and.fix = TRUE)
...
nz_shapefile <- read_web_sf(url = url, layer_id = layer_id)
nz_shapefile
```
...
We can visualise the location of the lake using the `tmap` package.
...
linz_basemap <- get_linz_basemap_tile(x = pnt, zoom = 17)
