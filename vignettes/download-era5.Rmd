...
```{r download-toba-osmdata, eval=FALSE}
library(osmdata)
...
Sometimes the `osmdata` package can be a bit tricky to use and may get rate
limit errors from the OpenStreetMap API. An alternative is to use the Nominatim 
API to download the shapefile for Lake Toba. This can be done using the
`st_read()` function.

```{r download-toba-nominatim, eval=TRUE, include=FALSE}
url <- "https://nominatim.openstreetmap.org/search?q=Lake+Toba&format=geojson&polygon_geojson=1"
...
Head over to the [AEME Inputs](https://limnotrack.com/AEME/articles/aeme-inputs.html) vignette to learn more about how to use the meteorological data to drive hydrodynamic models.
