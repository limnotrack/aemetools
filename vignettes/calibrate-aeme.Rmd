...
Using the `AEME` functions, we will build an ensemble of AEME models and run them.
...
aeme_data <- run_aeme(aeme_data = aeme_data, model = model,
                      ...
                      path = path)
# plot(aeme_data, model = model, path = path, plot = "calib",
...
as follows:
 ...
 - `vars_sim`: The variables to use for the calibration using the AEME
 variable definition e.g. `c("HYD_temp", "HYD_wlev")`.  

Here is an example of the control parameters:
```{r define-control-parameters}
ctrl <- list(VTR = 1, NP = 20, itermax = 200, reltol = 0.07,
...
```{r visualise-calibration-results}
