...
best_params <- get_param(calib, na_value = ctrl$na_value, fit_col = "fit", best = TRUE)
best_params

```

The best parameter values can be used to run the model and compare the simulated
values to the observed values. This can be done using the `run_aeme_param` function.
...
-   `aeme_data`: The `aeme_data` object which has observations and model
    simulations.
...
assess_model(aeme_data = aeme_data, model = model, var_sim = vars_sim)
...
the AEME package. This returns a list of ggplot objects, a plot of residuals
for each variable. This is a multi-panel plot displaying residuals for:
